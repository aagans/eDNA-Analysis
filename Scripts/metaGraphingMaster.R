...
bar_data <- c(as.integer(matrix_cluster[,2]), as.integer(matrix_month[,2]), as.integer(matrix_season[,2]), as.integer(matrix_year[,2]), as.integer(matrix_location[,2]))
bar_labels <- c(as.integer(matrix_cluster[,1]), matrix_month[,1], matrix_season[,1], as.integer(matrix_year[,1]), matrix_location[,1])

barplot(bar_data, 
        main = "Presence of Criteria in Selected ", 
        ...
        space = c(1, rep(.5, (length(matrix_cluster[,2]) - 1)),
                  2,
                  rep(.5, (length(matrix_month[,2]) - 1)),
                  2,
                  rep(.5, (length(matrix_year[,2]) - 1)),
                  2,
                  rep(.5, (length(matrix_location[,2]) - 1)))
